...
console.log('3rd hasing', createHashPW('1234')); // 전부 다 똑같은 값
console.log('another val', createHashPW('1234.')); // 전부 다 똑같은 값

// 1-2 ) pbkdf2Sunc(암호화할값, 솔트, 반복횟수, 키 길이, 알고리즘).toString(인코딩방식)
/* 
- 솔트를 이용해서 해싱하는 함수
...
// const str = 'hello!';
// const buffer = Buffer.from(str); // 100 001(2진수) 형태의 버퍼가 16진수로 표현
// console.log(buffer);
// 회원가입 과정
...
function checkPW(inputPW, savedSalt, savedHash) {
  // crypto.pbkdf2Sync;
  const iterations = 100; // saltAndHashPW 랑 같아야 함
  const keylen = 64; // slatAndHashPw 랑 같아야 함
  const algorithm = 'sha512'; // slatAndHashPw 랑 같아야 함
