...
@Component // 클래스에 해당 어노테이션을 달면, 스프링 컨테이너에서 관리할 수 있도록 해줌. like @bean(bean 은 메소드)
@Slf4j
// OncePerRequestFilter: 리퀘스트'당' 한 번 걸치는 필터
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        log.warn("filter session check {} {}", session.getId(), userId);
            ...
            if(userId != null) {
                ...
                // UsernamePasswordAuth~~~Token 토큰을 만들어서 SecurityContextHolder 에 담아
                // 컨트롤러까지 도달할 수 있도록 함
